...
file_test = "data/rec_log_test.txt"
...
def generateCandidatesWithWeights(sc):
	...
	model = ALS.train(train1, rank = 10, iterations = 3)
	...
	filetest = sc.textFile(file_test).map(parseRating2).cache()
	filetest_data = filetest.map(lambda x: (x[0], x[1]))
	predictions_test = model.predictAll(filetest_data).map(lambda r: (r[0],r[1],r[2]))
	predictions_test.coalesce(1, True).saveAsTextFile("subalg/user_item/test")